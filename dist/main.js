...
var getVars = () => {
  ...
  const targetDirs = targetPaths.map((p) => path__default.default.parse(p).dir);
  return {
    cacheDir,
    cachePath,
    options,
...
// src/main.ts
async function tryRestoreCache(cachePath, targetPath, targetDir, strategy) {
...
async function main() {
  try {
    ...
    if (!cacheHit) {
      log_default.info(`Skipping: cache not found for ${options.paths.join(", ")}.`);
    }
